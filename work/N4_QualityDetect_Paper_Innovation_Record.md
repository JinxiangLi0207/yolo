...
当前建议将 N4-QP3 作为论文主模型的第一个核心创新点，开发阶段配置固定为 `quality_power=3.0`。三种子统一校准结果证明其能够以很小的参数和计算开销稳定改善高 IoU 定位性能，但 Recall 平均下降约 0.026，且 scallop 的增益接近零并具有较大种子波动。N6-A 分支独立 power 已验证失败：降低 P3 power 会破坏跨尺度分数可比性，无法同时恢复 Recall 和保持 mAP50-95。下一算法实验转向全尺度一致的残差质量融合，不再叠加 MSFF；正式投稿前必须在独立 dev_val 上选择融合参数，并在冻结配置后评价 test。
