...
当前核心结论基于 DUO 数据集、YOLO11n、batch=96 的三随机种子实验。50 epochs 用于快速筛选，100 epochs 的 seeds 0/1/2 已完成正式配对验证。N4-Full 在三个种子上的 mAP50-95 分别提高 0.006、0.005 和 0.004，平均提高 0.005；增益稳定集中在高 IoU 定位质量，而非 mAP50 或 Recall。
...
```text
三种子平均 Precision： 0.834 -> 0.860，提升约 2.5 个百分点
三种子平均 Recall：    0.773 -> 0.750，下降约 2.3 个百分点
三种子平均 mAP50：     0.844 -> 0.843，下降约 0.1 个百分点
三种子平均 mAP50-95：  0.658 -> 0.663，提升约 0.5 个百分点
参数量增加：           约 0.45%
GFLOPs 增加：          约 1.6%
```

当前建议将 N4-Full 作为论文主模型的第一个核心创新点，正式配置固定为 `quality_power=1.0`。100 epochs 三种子结果证明其能够以很小的参数和计算开销稳定改善高 IoU 定位性能，但不能证明 mAP50、Recall 或所有类别全面改善。下一阶段应优先完成独立水下数据集泛化、统一环境速度测试、Jetson Nano 部署和可视化，而不是继续在 DUO test 上反复搜索模块。scallop 三种子平均 mAP50-95 下降约 0.010，必须作为方法限制分析，并可作为后续类别均衡质量学习的研究动机。
