...
def parse_model(d, ch, verbose=True):
    ...
    attn_modules = frozenset({SE, ECA, CA, SimAM, SK, BAM, A2, SLAM,
                              ...
                              MSBlock, InceptionDWConvBlock, C2f_MSBlock, MSBlock_S,
                              UFE, UFE_GAP, UFE_DC, UFE_G0})  # 逐步加
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        # ✅ 在这里加：注意力模块（输入输出通道不变，但构造函数需要 channels）
            ...
            c1 = ch[f] if isinstance(f, int) else ch[f[-1]]
            # 特殊处理 MSBlock 系列模块（输入输出通道可能不同）
            ms_modules = {'MSBlock', 'InceptionDWConvBlock', 'C2f_MSBlock', 'MSBlock_S'}
            if hasattr(m, '__name__') and m.__name__ in ms_modules:
                c2 = args[0]  # YAML中指定的是输出通道
                args = [c1, c2, *args[1:]]  # 注入输入通道
            else:
                c2 = c1
                args = [c1, *args]  # 注入 channels：SE(C, ...)
            ...
            if m in {Detect, QualityDetect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB}:
                m.legacy = legacy
